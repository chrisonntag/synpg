...
from utils import sent2str, synt2str, load_dictionary
import numpy as np
import torch
...
# Get environment variable for the device (cpu/cuda) and set cpu as default
DEVICE = os.environ.get("DEVICE", "cpu")
args = {
    'dataset_path': "../sst2-constituency",
...
def template2tensor(templates, max_tmpl_len, dictionary):
    ...
    tmpls = torch.from_numpy(tmpls).cpu()
...
def model_fn(model_dir):
    ...
    synpg_model.load_state_dict(torch.load(args['synpg_model_path'], map_location=torch.device(DEVICE)))
    synpg_model = synpg_model.cpu()
    synpg_model.eval()

    pg_model = SynPG(len(dictionary), 300, word_dropout=0.4)
    pg_model.load_state_dict(torch.load(args['pg_model_path'], map_location=torch.device(DEVICE)))
    pg_model = pg_model.cpu()
    pg_model.eval()
...
def predict_fn(input_data, model):
    ...
    with torch.no_grad():
        ...
        tagss = torch.from_numpy(tagss).cpu()
        ...
        synts = torch.from_numpy(synts).cpu()
        ...
        sents = torch.from_numpy(sents).cpu()
